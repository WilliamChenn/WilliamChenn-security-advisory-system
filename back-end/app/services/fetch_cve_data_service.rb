...
class FetchCVEDataService
  ...
  def self.call(vendor_name)
    fetch_data(vendor_name, 20)  # Default resultsPerPage is 20
  end

  def self.call_with_limit(vendor_name)
    fetch_data(vendor_name, 1)  # Custom resultsPerPage is 1
  end

  private

  def self.fetch_data(vendor_name, results_per_page)
    url = "#{BASE_CVE_API_URL}&vendorName=#{vendor_name}&pageNumber=1&resultsPerPage=#{results_per_page}"
