...
const Table = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response =  await fetch(`${backendUrl}/api/v3/cves/recent?time_range=year`, {
          credentials: 'include',
        });
        if (!response.ok) {
  ...
  const handleRefresh = async () => {
    setLoading(true);
    try {
      const response =  await fetch(`${backendUrl}/api/v3/vendors/refresh_vendors`, {
        method: 'POST',
        headers: {
      ...
      const updatedDataResponse =  await fetch(`${backendUrl}/api/v3/cves/recent?time_range=year`, {
        credentials: 'include',
      });
